...
from ..Util import input_controller_util
...
class InputController:
    ...
    def typewrite(
        ...
        if hWnd is not None:
            # 后台输入到指定窗口 - 使用 WM_CHAR 消息，不改变焦点
            try:
                for char in text:
                    char_code = ord(char)
                    # 发送 WM_CHAR 消息，这种方式不会改变窗口焦点
                    InputController._send_message_to_window(
                        hWnd, win32con.WM_CHAR, char_code, 0, debug, ignore
                    )
                    # 添加字符间延迟
                    if duration > 0 and len(text) > 1:
                        SystemController.sleep(
                            duration // len(text), debug=debug, ignore=ignore
                        )
            except Exception as e:
                if not ignore:
                    raise RuntimeError(
                        f"Failed to typewrite '{text}' to window {hWnd}: {e}"
                    ) from e
        else:
            # 前台输入
            try:
                SafeRunner.run(
                    pywinauto.keyboard.send_keys,  # type: ignore[unknown-attribute]
                    (text,),
                    {
                        "with_spaces": True,
                        "pause": duration / 1000 / len(text) if len(text) > 0 else 0,
                    },
                    ignore=ignore,
                    debug=debug,
                    # logger
    ...
    def keyboard_release(
        ...
        if hWnd is not None:
            ...
            for key in keys:
                ...
                if key_code:
                    InputController._virtual_key_to_window(
                        hWnd, key_code, "keyup", debug, ignore
                    )
                SystemController.sleep(sep_time, debug=debug, ignore=ignore)
        else:
            # 前台按键释放
    ...
    def keyboard_press_and_release(
        ...
        InputController.keyboard_press(
            keys, sep_time=sep_time, debug=debug, ignore=ignore, hWnd=hWnd
        )
    ...
    def mouse_move_to(
        ...
        if hWnd is not None:
            ...
            try:
                ...
                if not ignore:
                    raise RuntimeError(
                        f"Failed to move mouse to ({x}, {y}) in window {hWnd}: {e}"
                    ) from e
        else:
            # 前台鼠标移动
            SafeRunner.run(
    ...
    def mouse_event(
        ...
        if hWnd is not None:
            ...
            try:
                # 如果没有提供坐标，使用当前鼠标位置
                x, y = InputController.get_mouse_position()
                lParam = input_controller_util.trans_lParam(x, y)
                ...
                if not ignore:
                    raise RuntimeError(
                        f"Failed to {event_type.lower()} mouse button {button} in window {hWnd}: {e}"
                    ) from e
        else:
            # 前台鼠标事件
    ...
    def mouse_click_at(
        ...
        if hWnd is not None and x is not None and y is not None:
            ...
            try:
                ...
                if not ignore:
                    raise RuntimeError(
                        f"Failed to click at ({x}, {y}) in window {hWnd}: {e}"
                    ) from e
        else:
            # 前台鼠标点击
            SafeRunner.run(
